...
class Player extends Participant {
  ...
  updatePlayerMoney(winStatus) {
    if (winStatus === "player bust" || winStatus === "dealer high score") {
      this.money -= 1;
    } else if (winStatus === "dealer bust" || winStatus === "player high score") {
      this.money += 1;
    }
  }
...
class TwentyOneGame {
  ...
  start() {
    ...
    while (ready) { // eslint-disable-line no-unmodified-loop-condition
      ...
      while (true) {
        ...
        this.playerGameplay();
        if (!this.player.isBusted(this.bustValue)) {
          this.dealerGameplay();
        }

        this.roundResults();

        if (this.player.brokeOrRich()) break;
  ...
  playerGameplay() {
    while (true) {
      ...
      this.player.updateHandValue(this.deck);
      if (this.player.isBusted(this.bustValue)) break; // sometimes it is not accepting my choice to hit;
    }
  }

  dealerGameplay() {
    while (this.dealer.dealerWantsToHit(this.bustValue)) { // eslint-disable-line max-len
  ...
  The game is played in matches, which are made up of rounds.
...
You have: ${humanCards} (total value: ${this.player.currentHandValue})`);
  ...
  determineWinner() {
    if (this.player.currentHandValue >= this.bustValue) {
      return "player bust";
    } else if (this.dealer.currentHandValue >= this.bustValue) {
      return "dealer bust";
